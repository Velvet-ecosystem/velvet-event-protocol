...
"""Ghost CAN observation event contract for the packaged API."""
...
_FORBIDDEN_PAYLOAD_KEYS = {
    "action", "actuate", "actuation", "capability", "command", "executor",
    "executor_name", "hardware", "hardware_target", "shell", "token",
}
...
class GhostCanObservation:
    can_id: Union[int, str]
    data_hex: str
    signals: Mapping[str, Scalar] = field(default_factory=dict)
    ...
    def to_payload(self) -> Dict[str, Any]:
        ...
        payload.update(_REQUIRED_FLAGS)
        if self.timestamp is not None:
            payload["timestamp"] = float(self.timestamp)
        return payload

def build_ghost_can_observation_event(*, source: str, observation: GhostCanObservation, parent_event_id: Optional[str] = None, receipt_id: Optional[str] = None) -> VelvetEvent:
    if not isinstance(source, str) or not source.strip():
        raise ValueError("source must be a non-empty string")
    return VelvetEvent(source=source.strip(), event_type=GHOST_CAN_OBSERVATION_EVENT, payload=observation.to_payload(), metadata={"contract": GHOST_CAN_OBSERVATION_CONTRACT, "authority": "none", "public_demo": True}, parent_event_id=parent_event_id, receipt_id=receipt_id)

def validate_ghost_can_observation_event(event: Union[VelvetEvent, Mapping[str, Any]]) -> None:
    ...
    if forbidden:
        raise ValueError("ghost CAN observation contains forbidden authority fields: {}".format(sorted(forbidden)))
    for key, expected in _REQUIRED_FLAGS.items():
        if payload.get(key) is not expected:
            raise ValueError("ghost CAN payload {} must be {!r}".format(key, expected))
...
def _normalize_can_id(can_id: Union[int, str]) -> int:
